...
public class OrderServiceTest {
    ...
    void setUp() {
        ...
        expectedOrders.addAll(List.of(
                ...
                new Order("Suschi", 6.0,LocalDateTime.now().minusDays(6)),
    ...
    @Test
    void addOrder() {
        Order order = new Order("Schashlik", 100500,LocalDateTime.now().plusDays(5));
        orderService.addOrder(order);
        assertEquals(4,orderService.getSize());                             //проверить через размер листа
        assertEquals(order, orderService.getAllOrders().getLast());         //проверить через последний добавленьій елемент
    ...
    @Test
    void getOrderByName() {
        Order order =orderService.getOrderByName("Suschi");
